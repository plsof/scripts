...
-A INPUT -p tcp -m iprange --src-range 10.101.50.249-10.101.50.251 -m tcp --dport 22300 -m comment --comment "scanPlatform" -j ACCEPT
-A INPUT -p tcp -m iprange --src-range 10.101.50.139-10.101.50.140 -m state --state NEW -m multiport --dports 0:65522 -m comment --comment "scanBugsServer" -j ACCEPT
-A INPUT -s 10.25.172.133/32 -p tcp -m tcp -m multiport --dports 10050 -m comment --comment "zabbix" -j ACCEPT
-A INPUT -s 10.25.172.74/32 -p tcp -m tcp -m multiport --dports 10050 -m comment --comment "zabbix" -j ACCEPT
-A INPUT -s 10.25.171.147/32 -p tcp -m tcp -m multiport --dports 10050 -m comment --comment "zabbix" -j ACCEPT
-A INPUT -s 10.25.172.67/32 -p tcp -m tcp --sport 54505:54506 -m comment --comment "salt master" -j ACCEPT
-A INPUT -s 10.25.172.67/32 -p tcp -m tcp --dport 22300 -m comment --comment "jump" -j ACCEPT
-A INPUT -s 10.102.102.0/24 -p tcp -m tcp --dport 22300 -m comment --comment "S 4A" -j ACCEPT
...
for i in ${USER[*]}
do
    useradd ${i} -g sclc_ops
    echo "E58H-pR2l8]IxEYg" | passwd ${i} --stdin
done
